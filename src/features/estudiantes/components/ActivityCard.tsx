...
import type { JobItem, JobStatus } from '../types/dashboard.types'
...
export const ActivityCard = ({ item, status }: ActivityCardProps) => {
  return (
    <article className="rounded-2xl border border-[#ece8e1] bg-white p-4 shadow-[0_2px_10px_rgba(26,37,59,0.05)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_26px_rgba(26,37,59,0.10)]">
      <div className="flex items-start gap-4">
        ...
        <div>
          <h4 className="text-xl leading-none font-semibold text-slate-900">{item.title}</h4>
          <p className="mt-1 text-base text-slate-500">{item.company}</p>
        </div>
      </div>

      <p className="mt-3 text-xl font-semibold text-[#059669]">{item.salary}</p>
